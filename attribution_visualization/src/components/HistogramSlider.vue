...
//non linear scale for auto-zoom slider
const zoomSlider = computed(() => {
  return Math.pow(sliderPosition.value / zoomSliderResolution, 0.3 / 1) * zoomSliderResolution
})

const autoZoomText = computed(() => {
  const pct = Math.round((zoomSlider.value / (zoomSliderResolution / 100)) * 10) / 10
  return `Auto Zoom: ${pct}%`
})
